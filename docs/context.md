...
| 2026-08-29 | **Verified PYQ Acquisition & Ingestion Pipeline Mandate: "EMPTY BUT TRUE over FULL BUT FABRICATED"** | Synthetic or LLM-generated questions are strictly prohibited from being labeled as official PYQs. Implemented 8 source classifications, 5 source tiers, 10-point `verifySource()` gate, deterministic text/OCR extraction (`question_text_original` vs `question_text_display`), answer key provenance (`OFFICIAL`, `VERIFIED_SECONDARY`, `EXPERT_REVIEWED`, `UNVERIFIED`, `UNKNOWN`), SHA-256 immutability, automated CLI pipeline (`npm run pyq:*`), and machine-readable national coverage matrix across all 50 exams. | Trust & Verification Overhaul |
| 2026-08-29 | **Phase 0.5 G0 Real-Nurse Validation Milestone: Defer Heavy Retention Infra Until Loop is Proven** | Explicitly deferred Stage 2 (WhatsApp/SMS/Webhooks) in favor of **G0: Real-Nurse Test** with 5–10 Indian nurses. Validates the core North Star loop: `Discover → Know if I can apply → Prepare → Track → Return`. Focus shifts strictly to G0 execution: UI de-cluttering, authentic content & PYQ verification, action-oriented Maya guidance, and 8 core PostHog funnel metrics with <60s Time to First Value. | Product Strategy & Validation Gate |
...
| 2026-08-29 | 🏛️ Source-First Verified PYQ Acquisition & Ingestion Pipeline Established | Established full verified PYQ acquisition, verification, ingestion, normalization, storage, and UI execution system. Adopted **"EMPTY BUT TRUE over FULL BUT FABRICATED"** mandate. Configured 8 strict source classifications, 5-tier provenance hierarchy, 10-point `verifySource()` gate, deterministic question extraction preserving `question_text_original`, answer key provenance tracking (`OFFICIAL`, `VERIFIED_SECONDARY`, `EXPERT_REVIEWED`, `UNVERIFIED`, `UNKNOWN`), duplicate SHA-256 detection, incomplete paper accounting, CLI automation (`npm run pyq:*`), automated test suite, and machine-readable national coverage matrix across all 50 exams (`docs/pyq_architecture.md`, `docs/pyq_source_registry.md`, `docs/pyq_coverage.md`). | Shift from manual/static mock questions to an automated, cryptographically verified source-first repository with 100% provenance clarity. |
